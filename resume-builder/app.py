...
. Ensure each point starts with a strong action verb and highlights relevant skills and accomplishments,escriptions to ensure they are clear, impactful, and align closely with the job posting requirements without overstating qualification,no of characters in each point should be 135-150 character including spaces ,Ensure the start and end dates are in *Month in year/YYYY* format,if dates are not specified or has placeholders remove it from json,remove to be added and  not specified,make sure the experience is present in actual resume,don't hallucinate experience
        Response: {generated_output}
        Resume: {resume_content}
        Job Description: {job_description}
...
def combined_validation(final_output, sections):
    ...
    prompt = f""" 
        ...
        You are a strict validator and editor,Review and optimize the given resume, eliminate redundancies, avoid repeating experiences in projects or vice versa, ensure consistent tone and formatting, align content with the job description, and maintain clarity and professionalism,for every % symbol in the items, add a backslash (\) before it only if it is not already present. Return the entire resume in a format ready for direct LaTeX compilation, adhering strictly to the specified template and structure 
        ---
        #### General Validation Rules:
        1. Remove placeholders (e.g., "xxx", "yyy") or null/empty fields,not specified in resume placeholder.
...
def latex_validation(final_output,sections):
    prompt = f"""
You are a strict validator and editor,Review and optimize enerate a LaTeX-formatted resume based on the provided content,Ensure all sections are formatted consistently and adhere to standard LaTeX practices without including any comments, explanations, or additional text outside the required LaTeX code. ensuring it adheres to the following requirements:for every % symbol in the items, add a backslash (\) before it only if it is not already present. review and refine the entire latex resume in a format ready for direct LaTeX compilation, adhering strictly to the specified latex template and structure 
2. Ensure all sections personal detsils, Education, Work Experience, Projects, Skills) are well-defined and formatted consistently..
5. Return the entire resume in a format ready for direct LaTeX compilation, adhering strictly to the specified template 
